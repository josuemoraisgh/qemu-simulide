...
static uint64_t stm32_ADC_get_nbr_cycle_per_sample( Stm32Adc* s, uint8_t channel );
static uint8_t stm32_ADC_get_channel_number( Stm32Adc* s, uint8_t convert_number );
...
static void stm32_adc_clk_irq_handler(void *opaque, int n, int level)
{
    ...
    if(level) stm32_ADC_update_ns_per_sample(s); /* Only update the ns per sample if the IRQ is being set. */
}

// Routine which updates the ADC IRQ.
...
static void stm32_adc_start_conv( Stm32Adc* s )
{
    ...
    uint8_t channel = stm32_ADC_get_channel_number( s, 1 );
    ...
    if( channel == 16 ){
        ...
        s->ADC_DR = s->ADC_values[channel]&0xFFF;
    }
    s->ADC_SR  &= ~ADC_SR_EOC;
    ...
    if( s->conv_continuous == false )
        timer_mod( s->conv_timer, curr_time + stm32_ADC_get_nbr_cycle_per_sample( s, channel ) );
...
static void stm32_adc_conv_timer_expire_cont( void *opaque ) //Continuous
{
    uint64_t curr_time = qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL);
...
static uint8_t stm32_ADC_get_channel_number( Stm32Adc* s, uint8_t n )
{
    ...
    if     ( n <= 6 ) return ((s->ADC_SQR3 >> 5*(n-1) ) & 0x0000001F);
    else if( n <= 12) return ((s->ADC_SQR2 >> 5*(n-7) ) & 0x0000001F);
    else              return ((s->ADC_SQR1 >> 5*(n-13)) & 0x0000001F);
}
...
static uint64_t stm32_ADC_get_nbr_cycle_per_sample( Stm32Adc* s, uint8_t channel )
{
    ...
    if( channel >= 10 ) cycle = (s->ADC_SMPR1 >> 3*(channel-10)) & 0x00000007; //recover index cycle from ADC sample time register 1 (SMPR1)
    else                cycle = (s->ADC_SMPR2 >> 3*channel)      & 0x00000007; //recover index cycle from ADC sample time register 2 (SMPR2)
    // cycle(0-7) numbers of cycles correspondent (1.5, 7.5, 13.5, 28.5, 41.5, 55.5, 71.5, 239.5)
    return s->ns_per_sample[cycle];
}
...
{  
    /* check if number of conversion (ADC_SQR1[24 20]) greater than 1 */  
     if((new_value >> 20 & 0x0000000f) > 1) hw_error("Mode Single conversion is only implemented\n");
...
static void stm32_ADC_CR2_write(Stm32Adc *s,uint32_t new_value)
{
    ...
    if( s->ADC_CR2&ADC_CR2_SWSTART )
    {
      ...
      else stm32_hw_warn("ignore request ADC_CR2_SWSTART!\n");
    } 
...
{
   /* check ADC Enable */  
   if( !(s->ADC_CR2 & ADC_CR2_ADON) )  hw_error("Attempted to read from ADC_DR while ADC was disabled\n");

   if( s->ADC_SR & ADC_SR_EOC ) /* check conversion complete*/
   {           
...
static uint64_t stm32_adc_read(void *opaque, hwaddr offset, unsigned size)
{
    Stm32Adc *s = (Stm32Adc *)opaque;
...
static void stm32_adc_write(void *opaque, hwaddr offset, uint64_t value, unsigned size)
{
    Stm32Adc *s = (Stm32Adc *)opaque;
...
void stm32_ADC_update_ns_per_sample( Stm32Adc *s )
{
  ...
  //if( clk_freq ) //convert cycles to ns
  //{
      s->ns_per_sample[0] = (1000000000LL*  1.5)/clk_freq;
      s->ns_per_sample[1] = (1000000000LL*  7.5)/clk_freq;
      s->ns_per_sample[2] = (1000000000LL* 13.5)/clk_freq;
      s->ns_per_sample[3] = (1000000000LL* 28.5)/clk_freq;
      s->ns_per_sample[4] = (1000000000LL* 41.5)/clk_freq;
      s->ns_per_sample[5] = (1000000000LL* 55.5)/clk_freq;
      s->ns_per_sample[6] = (1000000000LL* 71.5)/clk_freq;
      s->ns_per_sample[7] = (1000000000LL*239.5)/clk_freq;
  //}
  //else{
  //    for( i=0; i<8; i++ ) s->ns_per_sample[i]=0;
  //}
}
